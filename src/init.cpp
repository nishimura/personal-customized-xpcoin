...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("XP version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  XPd [options]                     " + "\n" +
                  "  XPd [options] <command> [params]  " + _("Send command to -server or XPd") + "\n" +
                  "  XPd [options] help                " + _("List commands") + "\n" +
                  "  XPd [options] help <command>      " + _("Get help for a command") + "\n";
...
{
    uiInterface.ThreadSafeMessageBox(str, _("XP"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("XP"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: XP.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: XPd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 28192 or testnet: 17778)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 28191 or testnet: 18345)") + "\n" +
        ...
        "  -minerwait=<ms>        " + _("Set waiting miner thread to <ms> mili seconds (default: 500)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  XP is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "XP server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("XP"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of XP") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart XP to complete") << "\n";
